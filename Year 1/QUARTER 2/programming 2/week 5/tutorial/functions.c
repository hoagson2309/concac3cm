...
bool equal_strings_ci(const char *string1, const char *string2) {
...
date_t read_date(void) {
    date_t date;
    int matches = scanf("%d-%d-%d", &date.day, &date.month, &date.year);
    while (matches != 3 || date.year < 1900 || date.day < 1 || date.day > 31 || date.month < 1 || date.month > 12) {
        printf("Invalid date. Please enter a valid date (dd-mm-yyyy): ");
        matches = scanf("%d-%d-%d", &date.day, &date.month, &date.year);
      while (getchar() != '\n');  // clear input buffer
    }
    while (getchar() != '\n');  // clear input buffer
    return date;
