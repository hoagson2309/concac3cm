...
What is the best-selling month? Use an SQL comment to add this answer to your SQL file.*/
	SELECT MONTH(payment_date) AS period, SUM(amount) AS turnover
	FROM payment
...
--film id,title, and description.
	SELECT TOP(20) release_year, film_id, title, description
	FROM film
...
--result by lastname.
	...
	FROM customer 
	WHERE first_name IN ('Kelly', 'Edith') --WHERE (first_name = 'Kelly' OR first_name = 'Edith')
	ORDER BY last_name;
*/
...
-- To retrieve only the best selling day, add TOP 1 and order by turnover decending
	SELECT TOP 1 CAST(payment_date AS DATE) AS period,
		SUM(amount) AS turnover
	FROM payment
	GROUP BY CAST(payment_date AS DATE) --GROUP BY PHAI LAY GIONG SELECT KO NHET ten sau AS dc
	ORDER BY turnover DESC;					-- CON ORDER BY THI LAM DC



--28. Show the average rental duration (the difference between the rental date and the return_date).
...
--by year, month ascending, and then descending by number of rentals.
	...
	FROM rental
	GROUP BY customer_id, YEAR(rental_date), MONTH(rental_date)  --GROUP BY PHAI LAY GIONG SELECT KO NHET ten sau AS dc
	ORDER BY year, month ASC, num_of_rentals DESC;				-- CON ORDER BY THI LAM DC
